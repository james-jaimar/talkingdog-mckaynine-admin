...
import { AdminPaymentStatus } from "@/hooks/useAdminPayments";

interface AdminPaymentDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  totalDue: number;
  currentPayment?: AdminPaymentStatus;
  onSave: (payment: {
    amountPaid: number;
    paymentDate?: string;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          <DialogTitle>Admin Fee Payment - {monthLabel}</DialogTitle>
        </DialogHeader>
        
        <div className="grid gap-4 py-4">
          <div className="bg-muted/50 p-3 rounded-lg">
            <p className="text-sm text-muted-foreground">Total Admin Fees Due</p>
